...
const regPatient = async (req, res, next) => {
  try{
    ...
    let checkUser = await User.findOne({ nationalCode: user.nationalCode });
    if (checkUser) {
      const error = new Error("");
      error.statusCode = 422;
      error.message = "کد ملی قبلا ثبت شده است!";
      throw error;
    }
...
const update = async (req, res, next) => {
  ...
  try{
    ...
    let {patientId,userData,patientData} = req.body;
    


    //const user = await User.findById(_id);
    
    let editingPatient = await Patient.findById(patientId);
    let editingUser = await User.findById(editingPatient.user);
    let checkUser = await User.findOne({ $and:[ {nationalCode:userData.nationalCode}, {_id: { $ne: editingUser._id }}]});
    if (checkUser) {
      const error = new Error("");
      error.statusCode = 422;
      error.message = "کد ملی قبلا ثبت شده است!";
    ...
    if (userData.password != '') {
      userData.password = await bcrypt.hash(userData.password, 12);
        editingUser.password = userData.password;
    }
    editingUser.fname = userData.fname;
    editingUser.lname = userData.lname;
    editingUser.province = userData.province;
    editingUser.city = userData.city;
    editingUser.hcenter = userData.hcenter;
    editingUser.village = userData.village;
    editingUser.nationalCode = userData.nationalCode;
    editingUser.email = userData.email;
    editingUser.mobileNumber = patientData.phone.cellphone;
    editingUser.save().then(async () => {
      patientData.user = editingUser._id.toString();
        if (editingPatient) {
            let birthday = moment(editingPatient.birthday).locale('fa').format('YYYY/MM/DD');
            editingPatient.user = patientData.user;
            editingPatient.birthday = birthday;
            editingPatient.gender = patientData.gender;
            editingPatient.educationRate = patientData.educationRate;
            editingPatient.jobStatus = patientData.jobStatus;
            editingPatient.ecoStatus = patientData.ecoStatus;
            editingPatient.address = patientData.address;
            editingPatient.phone = patientData.phone;
