  ...
  try {
    ...
    if (!user) {
      const error = new Error(
        "A user with this nationalCode could not be found"
      );
    ...
    res.status(200).json({ token, user: user._id.toString(), statusCode: 200 });
    console.log("ss");
  } catch (err) {
    if (!err.statusCode) {
      err.statusCode = 500;
...
const update = async (req, res, next) => {
  try {
    ...
    let checkUser = await User.findOne({ $and:[ {nationalCode}, {_id: { $ne: _id }}]});
    if (checkUser) {
      const error = new Error("");
      error.statusCode = 422;
      error.message = "کد ملی قبلا ثبت شده است!";
...
const resetPassword = async (req, res, next) => {
  ...
  try {
    let forgetField = await ForgetPassword.findOne({ key: forget.token });

    if (!forgetField) {
      const error = new Error("");
      error.statusCode = 422;
      error.message = "توکن نامعتبر";
      throw error;
    }
    let editingUser = await User.findById(
      mongoose.Types.ObjectId(forgetField.user)
    );
    if (editingUser.email != forget.email) {
      const error = new Error("");
      error.statusCode = 422;
      error.message = "ایمیل وارد شده صحیح نیست";
      throw error;
    }

    forget.password = await bcrypt.hash(forget.password, 12);
    editingUser.password = forget.password;
    await editingUser.save();
    var key = crypto.randomBytes(20).toString("hex");
    let token = await bcrypt.hash(key, 12);
    forgetField.key = token;
    await forgetField.save();

    res.status(200).json({ message: "گذرواژه با موفقیت تغییر یافت!" });
  } catch (err) {
    next(err);
  }
};
const passwordReset = async (req, res, next) => {
  ...
  res.render("panel/password_reset", {
...
const checkUserExist = async (req, res, next) => {
  ...
  const userByNationalCode = await User.findOne(
    { $and:[ {nationalCode:req.body.nationalCode}, {_id: { $ne: req.body.userId }}]});
  checkData = {
